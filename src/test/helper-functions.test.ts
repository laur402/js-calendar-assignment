...
import {
    addDays,
    addHours, cycleArray, getDayDifference, getDaySpan,
    getFirstDayOfMonth,
    getFirstDayOfWeek,
    getLastDayOfMonth,
    getLastDayOfWeek,
    getNormalizedLocalDate, getTimePercentageOfDay, getTimezone, getWeekDifference,
    leftPad
...
describe("Date-outputting functions", ()=>{
    describe(getFirstDayOfWeek, ()=>{
        ...
        test("specify first day of the week", ()=>{
            const testDate = new Date("2025-07-17");
            expect(getFirstDayOfWeek(testDate, WeekDays.Sunday)).toEqual(new Date("2025-07-13"))
        });
    });
    describe(getLastDayOfWeek, ()=>{
        ...
        test("specify last day of the week", ()=>{
            const testDate = new Date("2025-07-17");
            expect(getLastDayOfWeek(testDate, WeekDays.Saturday)).toEqual(new Date("2025-07-19"))
        });
    });
    ...
    describe(getNormalizedLocalDate, ()=>{
        ...
        test("base functionality", ()=>{
            ...
            expect(getNormalizedLocalDate(testDate)).toEqual(new Date("2025-07-17 00:00"));
        });
...
describe(getTimezone, ()=>{
    test("base functionality", ()=>{
        ...
        expect(getTimezone(testDate)).toBe(Number(testDate.toString().split("GMT")[1].slice(0, 3)))
    });
});
...
describe(cycleArray, ()=>{
    ...
    test("cycleBy inputs with same remainders from array length should result in same arrays", ()=>{
        const testArray = ["A", "B", "C", "D", "E"];
        expect(cycleArray(testArray, testArray.length+3)).toEqual(cycleArray(testArray, 2*(testArray.length)+3));
    });
})
