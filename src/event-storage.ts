...
export async function modifyEvent(event: CalendarEvent) {
    ...
    const value: Response = await fetch(
        `http://localhost:3000/events/${APIEvent.id}`,
        {
            method: 'PUT',
...
export async function getEvent(eventId: string): Promise<CalendarEvent | null> {
    const eventFetch: Response = await fetch(
...
export async function fetchEvents(): Promise<CalendarEvent[]> {
    const eventsFetch: Response | null = await asyncTryCatch(
        () => fetch('http://localhost:3000/events'),
        null,
        () => alert('Events server is unavailable!'),
    );
    if (eventsFetch === null) return [];

    if (eventsFetch.status !== 200)
        throw new Error('Invalid event server link');
